...
while True:
    ...
    if not first_frame:
        good_matches, prev_good_keypoints, curr_good_keypoints = mono_VO.compute_matches(
            prev_descriptors,
            curr_descriptors,
            prev_keypoints,
            curr_keypoints,
        ...
        matched_img = cv.drawMatches(
            prev_gray, prev_keypoints, gray, curr_keypoints, good_matches, None, **draw_params 
        )
        
        cv.imshow("Matches", matched_img)
        
        trajectory = mono_VO.compute_trajectory(
            prev_good_keypoints, curr_good_keypoints
        )
